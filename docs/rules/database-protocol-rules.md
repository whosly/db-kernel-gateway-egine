...
- MySQL capability flag 白名单：需要结合第一阶段真实支持能力列出默认开启集合。
- MySQL error code / SQLSTATE 映射表：当前规则要求存在映射，后续实现应定义首批
  错误映射表。
- PostgreSQL type OID 映射表：当前规则要求集中维护，后续实现应定义首批类型集合。
- PostgreSQL Extended Query 错误恢复：规则已有 Sync 恢复要求，后续实现计划要明确
  Parse、Bind、Execute 每一步的错误状态。
...
握手或启动阶段对客户端公开的能力必须真实可用。禁止为了“兼容”而开启未实现
的 capability flag、认证插件、压缩、TLS、二进制结果格式或 extended query
能力。需要未来支持的能力应写入能力模型和测试计划，但默认不向客户端广告。
...
- 如果服务端没有在 Initial Handshake 中声明 `CLIENT_SSL`，客户端仍发送
  SSLRequest，应按 malformed/unsupported startup 处理并关闭连接。
...
服务端握手包中的 auth plugin name 必须与实际认证路径一致。如果只实现
`mysql_native_password`，不得同时宣称完整支持 `caching_sha2_password`。
如果后续支持 `caching_sha2_password`，必须实现完整认证继续流程或明确返回
auth switch / auth more data 的受限路径。
...
ParameterStatus 的值必须与网关实际会话行为一致。不能固定返回与后端或网关
编码、时区、字符串处理不一致的值；如果暂时无法精确透传，必须选择保守默认并
在实现或测试中记录兼容性限制。
...
RowDescription 的 format code 必须与 DataRow 编码一致。第一阶段如果只支持
文本格式，则所有字段 format code 应为 `0`，并且 Bind 中请求 binary format 时
必须返回明确错误或协商为文本格式。
